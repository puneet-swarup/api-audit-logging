...
class CleanupConfigTest {
  ...
  private final ApplicationContextRunner contextRunner =
      new ApplicationContextRunner()
          ...
          .withBean(ApiAuditLogRepository.class, () -> Mockito.mock(ApiAuditLogRepository.class))
  ...
  void cleanupDisabled() {
    contextRunner
        ...
        .run(context -> Assertions.assertThat(context).doesNotHaveBean(CleanupConfig.class));
  }
  ...
  void cleanupEnabled() {
    contextRunner
        ...
        .run(context -> Assertions.assertThat(context).hasSingleBean(CleanupConfig.class));
  }
    ...
    contextRunner
        ...
        .run(
            context -> {
              ...
              Mockito.verify(repository).deleteByTimestampBefore(cutoffCaptor.capture());

              LocalDateTime actualCutoff = cutoffCaptor.getValue();
              LocalDateTime expectedCutoff = LocalDateTime.now().minusDays(10);

              Assertions.assertThat(actualCutoff)
                  .isCloseTo(expectedCutoff, Assertions.within(1, ChronoUnit.MINUTES));
            });
  }
    ...
    contextRunner
        ...
        .run(
            context -> {
              ...
              Assertions.assertThat(cutoffCaptor.getValue())
                  .isCloseTo(expectedDefault, Assertions.within(1, ChronoUnit.MINUTES));
            });
  }
}
