package java.com.api.audit.listener;
...
class ApiLogListenerTest {
  ...
  void testHandleLog() {
    ...
    Mockito.when(jsonMasker.mask("{\"password\":\"123\"}")).thenReturn("{\"password\":\"******\"}");
    Mockito.when(jsonMasker.mask("{\"token\":\"456\"}")).thenReturn("{\"token\":\"******\"}");

    listener.handleLog(event);

    Mockito.verify(jsonMasker, Mockito.times(2)).mask(ArgumentMatchers.any());
